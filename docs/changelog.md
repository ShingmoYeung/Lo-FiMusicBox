...
## v1.0.1（macOS 27 拖动兼容性修复）

定位：修复系统升级到 macOS 27 后，已安装应用无法拖动无标题栏主窗口的问题，并同步发布版本元数据。

### 修复

- 修复 macOS 27 下拖动木质机身无法移动主窗口的问题。
- macOS 15 及以上版本使用 `WindowDragGesture` 显式处理窗口拖动，同时为窗口形状补充一致的命中区域。
- 保留 macOS 14 的 `WindowConfigurator` AppKit 背景拖动回退，兼容原有最低支持系统。
- 保持音量滑杆、播放按钮和频道按钮的独立交互，不让窗口拖动手势覆盖控件操作。

### 发布

- 应用营销版本更新为 `1.0.1`，构建版本更新为 `2`。
- 更新 README 版本徽章与发布标签说明。
